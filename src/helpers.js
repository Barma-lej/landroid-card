...
/**
 * Checks if the stats configuration needs to be migrated
 * 
 * @param {*} statsConfig - The stats configuration to check
 * @returns {boolean} - True if the stats configuration needs to be migrated, false otherwise
 */
export function needsStatsMigration(stats) {
  if (!stats) return false;
  // Если старый формат: объект вида { default: [...], mowing: [...] }
  if (!Array.isArray(stats) && typeof stats === 'object') return true;
  // Если массив, но содержит устаревшие ключи (entity_id, subtitle, value_template)
  if (Array.isArray(stats)) {
    return stats.some(
      (item) =>
        item &&
        ('entity_id' in item || 'subtitle' in item || 'value_template' in item)
    );
  }
  return false;
}
 ...
 * 
 * @param {*} statsConfig - The stats configuration to migrate
 * @returns {Array} - The migrated stats configuration
 */
export function migrateStats(stats) {
  if (!stats) return [];

  // 1. Старый формат (объект с группами по статусам)
  if (!Array.isArray(stats) && typeof stats === 'object') {
    const result = [];
    for (const [stateKey, items] of Object.entries(stats)) {
  ...
  // 2. Если уже массив, но со старыми названиями полей
  if (Array.isArray(stats)) {
    return stats.map((item) => ({
      ...
      ...(item.template || item.value_template
        ? { template: item.template || item.value_template }
        : {}),
      states: item.states || (item.state ? [item.state] : ['default']),
    }));
  }
